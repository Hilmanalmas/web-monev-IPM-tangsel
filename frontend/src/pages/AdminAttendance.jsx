...
const AdminAttendance = () => {
    ...
    const [form, setForm] = useState({ name: '', start_time: '', end_time: '', day: 1 });
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 p-6">
            <h1 className="text-3xl font-black flex items-center gap-3">
                <Clock className="text-blue-500" size={32} /> Pengaturan Jadwal Absensi
            </h1>

            <div className="bg-white p-6 rounded-3xl shadow-lg border-2 border-blue-50">
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                    <Plus className="text-blue-500" /> Tambah Jadwal Baru
                </h2>
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="md:col-span-2">
                        <label className="text-xs font-bold text-gray-400 uppercase">Nama Sesi</label>
                        <input type="text" placeholder="Pagi / Siang / Materi 1..." required className="w-full p-3 border rounded-xl"
                    ...
                    <div className="flex items-end">
                        <button type="submit" className="w-full bg-blue-600 text-white p-3 rounded-xl font-black hover:bg-blue-700 active:scale-95 transition-all">
                            SIMPAN
                        </button>
                    </div>
                    <div>
            ...
            <div className="space-y-6">
                {Object.keys(groupedSlots).sort().map(day => (
                    <div key={day} className="space-y-3">
                        <h3 className="text-lg font-black text-gray-800 flex items-center gap-2 px-2">
                            <Calendar size={20} className="text-blue-500" /> HARI {day}
                        </h3>
                ...
                {slots.length === 0 && (
                    <div className="text-center py-20 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-gray-400 font-bold">Belum ada jadwal yang diatur.</p>
                    </div>
                )}
            </div>
        </div>
